...
def preprocess_df(
    ...
    df.rename(
        columns={prev: new for prev, new in zip(prev_names, new_names)}, inplace=True
    )
...
if __name__ == "__main__":
    ...
    considered_diseases = [
        "disease_" + "_".join(x.split("_")[1:]) for x in _considered_diseases
    ]
    ...
    considered_symptoms = [
        "symptom_" + "_".join(x.split("_")[1:]) for x in _considered_symptoms
    ]
    ...
    disease_state_dict = {
        k: {"state_names": ["False", "True"], "dtype": "binary"}
        for k in considered_diseases
    }
    symptom_state_dict = {
        k: {"state_names": ["False", "True"], "dtype": "binary"}
        for k in considered_symptoms
        if not k == "symptom_temp"
    }
    fever_bins = [35, 37, 40, 44]
    fever_state_names = [
        str(x) + "-" + str(y) for x, y in zip(fever_bins[:-1], fever_bins[1:])
    ]
    symptom_state_dict["symptom_temp"] = {
        "state_names": fever_state_names,
        "vals": fever_bins,
        "dtype": "continuous",
    }
        ...
        for base_feature in base_features:
            ...
            if base_feature_dict.dtype == "continuous":
                ...
                for i in range(len(states) - 1):
                    state_interval_inds = (df[base_feature] > states[i]) & (
                        df[base_feature] <= states[i + 1]
                    )
                    prob_disease_given_base_bin = sum(
                        state_interval_inds & pos_disease_inds
                    ) / sum(state_interval_inds)
                    prob_disease_given_base_bins.append(prob_disease_given_base_bin)

                if not all([v == 0 for v in prob_disease_given_base_bins]):
                    base_feature_disease_prob_dict = update_dict(
                        base_feature_disease_prob_dict,
                        base_feature,
                        disease,
                        prob_disease_given_base_bins,
                ...
                for state in states:
                    ...
                    prob_disease_given_state = sum(df[state_inds][disease] == 1) / sum(
                        state_inds
                    )
                    prob_no_disease_given_state = 1 - prob_disease_given_state
                    assert (prob_disease_given_state <= 1) & (
                        prob_no_disease_given_state <= 1
                    ), f"prob of having/ not having {disease} greater than 1"
    ...
    for symptom in tqdm(considered_symptoms):
        ...
        for disease in considered_diseases:
            if symptom_dict["dtype"] == "binary":
                ...
                pos_inds = inds == 1
                assert np.all(
                    np.array(df[pos_inds][disease]) == 1
                ), "not all negative indices correspond to negative cases"

                # proportion of patients with disease that also have symptom
                prob_symptom_with_disease = sum(df[pos_inds][symptom] == 1) / sum(
                    pos_inds
                )
                assert (
                    prob_symptom_with_disease <= 1
                ), f"prob of having {symptom} greater than 1"

                if prob_symptom_with_disease > 0:
                    disease_symptom_prob_dict = update_dict(
                        disease_symptom_prob_dict,
                        disease,
                        symptom,
                        prob_symptom_with_disease,
                    )
                ...
                for i in range(len(fever_bins) - 1):
                    fever_inds = (df[symptom] > fever_bins[i]) & (
                        df[symptom] <= fever_bins[i + 1]
                    )
